...
void CKKSTaskSolver::loadModelWeights() {
    try {
        ...
        // Load fc1 weights (13x32)
        if (model_data.contains("fc1.weight")) {
            auto& w1 = model_data["fc1.weight"];
            for (int i = 0; i < hidden1; i++) {  // For each output neuron
                for (int j = 0; j < input_dim; j++) {  // For each input feature
                    // Note the transposition here - PyTorch stores weights as [out_features, in_features]
                    weights1[j][i] = w1[i][j];
                }
            }
        }
        ...
        // Load fc3 weights (32x1) - Note this is directly connected to fc1, there is no fc2
        if (model_data.contains("fc3.weight")) {
            auto& w3 = model_data["fc3.weight"];
            for (int i = 0; i < output_dim; i++) {  // For each output neuron
                for (int j = 0; j < hidden1; j++) {  // For each input feature
                    weights3[j][i] = w3[i][j];
                }
            }
        }
        ...
        // Load activation function parameters
        if (model_data.contains("poly_act.beta")) {
            beta = model_data["poly_act.beta"];
        }
        
        if (model_data.contains("poly_act.gamma")) {
            gamma = model_data["poly_act.gamma"];
        }
...
Ciphertext<DCRTPoly> CKKSTaskSolver::activation(const Ciphertext<DCRTPoly> &input) {
    ...
    // Calculate x^2
    Ciphertext<DCRTPoly> x_squared = m_cc->EvalMult(input, input);
